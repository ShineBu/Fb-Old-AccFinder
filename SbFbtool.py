...
if sys.version_info[0] != 3:
    print('''\t--------------------------------------\n\t\tYou Should Install PYTHON 3.x\n\t\tPlease install and try: python3 
    sbfb.py\n\t--------------------------------------''')
    sys.exit()

PASSWORD_FILE = "phoneNumbers.txt"
...
if __name__ == "__main__":
    print('\n---------- Welcome To Facebook Old App Seaching ----------\n')
    if not os.path.isfile(PASSWORD_FILE):
        print("Password file is not exist: ", PASSWORD_FILE)
        sys.exit(0)
    password_data = open(PASSWORD_FILE, 'r').read().split("\n")
    print("Selected Phone Number As Password: ", PASSWORD_FILE)
    print("Seleted Phone Number As Email: " , PASSWORD_FILE)
    email = PASSWORD_FILE #input('Enter Email/Username to target: ').strip()
